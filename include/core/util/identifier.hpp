...
public:
    Identifier(std::string _namespace, std::string _path): 
        _namespace(std::move(_namespace)), _path(std::move(_path)) {
            if (!verifyNamespace(_namespace) || !verifyName(_path)) 
                throw std::runtime_error(std::string("Error constructing Identifier: ") + _namespace + " " + _path);
        }
    ...
    inline static Identifier parse(const std::string& str) {
        ...
        if (elems.size() != 2) {
            throw std::runtime_error(std::string("Error parsing Identifier, not separated by exactly one colon: ") + str);
        }
        ...
        if (!verifyNamespace(_namespace) || !verifyName(_path)) 
            throw std::runtime_error(std::string("Error parsing Identifier: ") + _namespace + " " + _path);
