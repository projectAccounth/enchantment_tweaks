...
struct Element {
    Vec3 from{};
    Vec3 to{};
    std::map<std::string, Face> faces;  // keyed by "north", "south", "up", "down", "west", "east"
};
...
private:
    ...
    bool isSolid(const std::vector<std::vector<bool>>& occupied, int x, int y) const;

    static std::optional<Element> makeElement(
