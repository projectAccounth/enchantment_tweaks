...
public:
  ...
  bool download(const std::string &url, DownloadTarget target,
...
private:
  NetworkManager() { curl_global_init(CURL_GLOBAL_DEFAULT); }

  ~NetworkManager() { curl_global_cleanup(); }

  static size_t stringWriteCallback(void *contents, size_t size, size_t nmemb,
                                    void *userp) {
  ...
  static size_t fileWriteCallback(void *ptr, size_t size, size_t nmemb,
                                  void *stream) {
    return std::fwrite(ptr, size, nmemb, static_cast<FILE *>(stream));
  }

  /// s1mple progress bar
  static int progressCallback([[maybe_unused]] void *clientp,
