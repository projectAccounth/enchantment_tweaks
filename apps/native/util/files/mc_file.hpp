...
static inline bool _fetchMcClient(
...
)
{
    ...
    try {
        ...
        try {
            ...
            std::cerr << "Failed to parse version manifest: "
                      << e.what() << "\n";
            return false;
        }
        ...
        if (!net.download(versionMetadataUrl, &versionData)) {
            std::cerr << "Failed to download metadata for "
                      << versionId << "\n";
            return false;
        }
        ...
        try {
            ...
            std::cerr << "Failed to parse version metadata: "
                      << e.what() << "\n";
            return false;
        }
        ...
        if (!versionJson.contains("downloads") ||
            ...
            std::cerr << "No client download found for "
                      << versionId << "\n";
            return false;
        }
        ...
        const fs::path metadataPath =
            outputDirectory / (versionId + ".json");
        ...
        // downloading the JAR
        const fs::path jarPath =
            outputDirectory / (versionId + ".jar");

        const fs::path tempJarPath =
            outputDirectory / (versionId + ".jar.tmp");

        fs::remove(tempJarPath, ec);

        std::cout << "Downloading Minecraft "
                  << versionId << "...\n";
        ...
        if (ec) {
            std::cerr << "Failed to finalize JAR: "
                      << ec.message() << "\n";
...
inline fs::path tryGetMcClient(const std::string& versionId) {
    try {
        ...
        if (!fs::exists(versionJar)) {
            std::cout << "Version JAR for " << versionId << " not found. Downloading.\n";
        ...
        exitProgram(ExitStatus::ERROR, "This executable might have executed at a location with insufficient permissions. Move it somewhere else or execute it as superuser/administrator.");
    }
}
