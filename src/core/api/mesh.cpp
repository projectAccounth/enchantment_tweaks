...
bool MeshBuilder::isSolid(const std::vector<std::vector<bool>>& occupied, int x, int y) const {
...
std::optional<Element> MeshBuilder::makeElement(
    ...
    std::optional<UV> uvNorth, std::optional<UV> uvSouth, std::optional<UV> uvUp,
    std::optional<UV> uvDown, std::optional<UV> uvWest, std::optional<UV> uvEast,
...
void MeshBuilder::appendPerPixelElements(const BuildOptions& options, double z1, double z2,
...
void MeshBuilder::appendMergedElements(const BuildOptions& options, double z1, double z2,
                                        std::vector<Element>& out) const {
    const int w = image_.width();
    const int h = image_.height();
    const double scaleX = MODEL_UNITS / w;
...
void MeshBuilder::appendOutlineElements(const BuildOptions& options, double z1, double z2,
...
Model MeshBuilder::Build(const BuildOptions& options) const {
    ...
    Model model;
    const std::string textureRef = options.textureNamespace + ":" + options.textureKey;
