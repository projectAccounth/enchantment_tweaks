...
Image Image::Load(const std::string& path) {
    ...
    if (!data) {
        ...
        throw std::runtime_error("enchantment_tweaks::image::Image::Load: failed to load '" +
                                  path + "': " + (reason ? reason : "unknown error"));
    }
...
Pixel Image::at(int x, int y) const {
    if (x < 0 || x >= width_ || y < 0 || y >= height_) {
        throw std::out_of_range("enchantment_tweaks::image::Image::at: (" + std::to_string(x) +
                                 ", " + std::to_string(y) + ") out of range");
    }
    const size_t idx = (static_cast<size_t>(y) * width_ + x) * 4;
    return Pixel{pixels_[idx + 0], pixels_[idx + 1], pixels_[idx + 2], pixels_[idx + 3]};
}
